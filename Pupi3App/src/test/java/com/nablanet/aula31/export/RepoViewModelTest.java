...
public class RepoViewModelTest {
    ...
    public void getDataSummary() {
        ...
        //when(viewModel.getCourseExtLiveData(dataParams.getCourseId())).thenReturn(courseExportMutableLiveData);
    ...
    public void getDataTrack() {
        ...
        classTrack.setObservations(new HashMap<String, Observation>());
        classTrack.getObservations().put("ownUid", new Observation());

        MemberTrack memberTrack1 = new MemberTrack();
        memberTrack1.setClasses(new HashMap<String, ClassTrack>());
        memberTrack1.getClasses().put("class1", classTrack);
    ...
    public void builDataTrack() {
        ...
        classTrack2.setObservations(new HashMap<String, Observation>());
        member5.getClasses().put("clase2", classTrack2);
        //Assert.assertNull(viewModel.postDataTrack(memberTrackMap, dataParams));

        // Si al miembro Nº5 le agregamos una observación de otros usuarios
        classTrack2.getObservations().put("user1uid", new Observation());
        classTrack2.getObservations().put("user2uid", new Observation());
        //Assert.assertNull(viewModel.postDataTrack(memberTrackMap, dataParams));

        // Si al miembro Nº5 le agregamos una observación nuestra sin fecha y no ponemos
        // límites de tiempo a la consulta
        classTrack2.getObservations().put("ownUid", new Observation());
    ...
    private HashMap<String, MemberTrack> getMemberTrackMap() {
        ...
        memberTrack.setClasses(new HashMap<String, ClassTrack>());
