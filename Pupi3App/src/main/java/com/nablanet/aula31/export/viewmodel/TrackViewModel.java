                ...
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (String memberId : dataParams.getMemberIdList()) {
                            ...
                            if (memberTrack == null || memberTrack.getClasses() == null)
                                continue;

                            List<DataTrackImpl.DayImpl> dayList = new ArrayList<>();
                            Observation observation;

                            for (ClassTrack classTrack : memberTrack.getClasses().values()) {
                                if (classTrack == null)
                                    continue;

                                boolean dayCondition = Util.isBetween(
                                        dataParams.getFrom(), classTrack.getDate(), dataParams.getTo()
                                );
                                ...
                                if (dayCondition && observation != null)
                                    dayList.add(new DataTrackImpl.DayImpl(
                                            classTrack.getDate(), observation.getRate(), observation.getComment()
                                    ));
